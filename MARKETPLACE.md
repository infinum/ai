...
| [`security-check`](plugins/security-check/)          | Dependency & package security toolkit — three skills: audit a system/OS or globally-installed package before installing it (`system-package-audit`), triage reported vulnerabilities in a project's dependencies for SOC2 deadlines (`vulnerability-report-triage`), and vet a new dependency before adopting it (`dependency-adoption-review`).                    | Not ready — needs local package managers, `gh`, and registry/advisory egress | `/plugin install security-check@infinum-ai`    |
...
| [`ai-ready-figma`](plugins/ai-ready-figma/) | Make Figma design systems and screen files AI-readable for MCP/Code-Connect code generation. Ships two skills: `ai-ready-design-system` (component library — token architecture, naming, property wiring, structural consistency) and `ai-ready-screens` (assembled screens/user-flows — frame naming, prototype-reaction flow graph, state coverage). Platform-neutral; pair with `design-conventions` for a framework binding. | Ready with the Figma **remote** MCP connector — both skills' build/audit writes run through `use_figma` (write-to-canvas), a remote-server tool (`mcp.figma.com`, OAuth) reached over the agent's MCP transport, not the local Dev Mode bridge; enable the Figma remote connector in the Cowork workspace (pending live verification). The naming/architecture guidance itself reads fine anywhere | `/plugin install ai-ready-figma@infinum-ai` |
| [`design-conventions`](plugins/design-conventions/) | Platform-binding conventions for `ai-ready-figma` design systems. Ships `flutter-m3-conventions` (Flutter Material 3) — the casing, `ColorScheme`/`TextTheme` mapping, state vocabulary, and code-syntax keys that Figma → Flutter code generation needs. Load alongside `ai-ready-design-system`; add a skill per platform as more are needed. | Ready — pure guidance (no MCP or local tooling required) | `/plugin install design-conventions@infinum-ai` |
| [`purpose-driven-design`](plugins/purpose-driven-design/) | Act as an Infinum Purpose Driven Design (PDD) practitioner — drawing on the PDD philosophy, principles, 6-phase process, and full toolkit. Skills: discovery workshops, interview guides, archetypes, rainbow & competitive analysis. | Ready — all 12 skills work in Cowork: the conversation/Markdown skills, the `.docx`/`.xlsx` generators (python-docx/openpyxl on Cowork's built-in office toolchain), and the FigJam ones via the Figma **remote** MCP connector (`create_new_file`/`use_figma` are remote write tools). `pdd-archetypes` requires that connector be enabled since its only output is a FigJam board (pending live verification); `pdd-discovery-workshop` builds its board the same way but also keeps a Markdown fallback. The PDD persona (`instructions.md`) is loaded by each skill at invocation — there is no SessionStart hook — so it applies in Cowork too. | `/plugin install purpose-driven-design@infinum-ai` |

See each skill's README for detailed usage and examples.

> **Renamed:** `package-security-check` is now the `security-check` plugin (it grew from one skill to three). If you have the old plugin installed, run `/plugin uninstall package-security-check@infinum-ai` then `/plugin install security-check@infinum-ai`. The old `/package-security-check` command is now `/security-check:system-package-audit`; the new skills are `/security-check:vulnerability-report-triage` and `/security-check:dependency-adoption-review`.

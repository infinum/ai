...
| [`purpose-driven-design`](plugins/purpose-driven-design/) | Act as an Infinum Purpose Driven Design (PDD) practitioner — drawing on the PDD philosophy, principles, 6-phase process, and full toolkit. Skills: discovery workshops, interview guides, archetypes, rainbow & competitive analysis. | Partially ready — 9 of 12 skills run in Cowork (conversation/Markdown skills, plus those using Cowork's built-in docx/xlsx skills). Not ready: `pdd-archetypes` (FigJam via the local Figma bridge) and the two interview-guide skills (`node`-based .docx generators). The persona SessionStart hook runs in Cowork too — pending live verification. | `/plugin install purpose-driven-design@infinum-ai` |

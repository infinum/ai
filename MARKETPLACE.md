...
| [`purpose-driven-design`](plugins/purpose-driven-design/) | Act as an Infinum Purpose Driven Design (PDD) practitioner — drawing on the PDD philosophy, principles, 6-phase process, and full toolkit. Skills: discovery workshops, interview guides, archetypes, rainbow & competitive analysis. | Partially ready — 11 of 12 skills run in Cowork: the conversation/Markdown skills plus the `.docx`/`.xlsx` generators (python-docx/openpyxl on Cowork's built-in office toolchain). Not ready: `pdd-archetypes` only — its sole output is a FigJam board via the local Figma bridge, with no fallback. (`pdd-discovery-workshop` is ready: its `.docx` run-of-show works and the FigJam step falls back to a Markdown scaffold.) The PDD persona (`instructions.md`) is loaded by each skill at invocation — there is no SessionStart hook — so it applies in Cowork too. | `/plugin install purpose-driven-design@infinum-ai` |
